...
void sets(string cards[], int suit[], int value[], string out)
{
  ...
  //  cout << "RUN of 4: " << endl;
    ...
    {
        if(value[a] == (value[a-3] - 3) && suit[a] == suit[a-3] && value[a] != 20)
        {
            for(int c = a; c > a - 4; c--)
            {
                out += cards[c] + ", ";
  ...
  //  cout << "RUN of 3: " << endl;
    ...
    for(int a = NUM_CARDS - 1; a >= 2; a--)
    {
        if(value[a] == (value[a-2] - 2) && suit[a] == suit[a-2] && value[a] != 20)
        {
            for(int c = a; c > a - 3; c--)
            {
                if(value[c] > (out[0] - '0') || number == 0)
                {
                    out+= cards[c] + ", ";
                    value[c] = 20;
                    suit[c] = 10;
  ...
  //  cout << "SET of 3: " << endl;
    ...
    cout << endl << "OUTPUT: " << out << endl << endl << endl;
}
